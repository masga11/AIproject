...
// Эндпоинт импорта дебатов
app.post('/memory/import', async (req, res) => {
  await globalMemory.init()

  const { debates } = req.body

  if (!debates || !Array.isArray(debates) || debates.length === 0) {
  ...
  for (const debate of debates) {
    try {
      if (!debate.topic || !debate.messages || !Array.isArray(debate.messages)) {
        skipped++
        continue
      }

      const topic = debate.topic.trim()
      const provider = debate.model || debate.provider || 'imported'
      const model = debate.model || 'unknown'
      const rounds = debate.rounds || 3

      const debateId = globalMemory.saveDebate(
        topic,
        provider,
        model,
        rounds,
        ...
        if (msg.agent && content) {
          globalMemory.saveMessage(
            debateId,
            msg.agent || msg.agentName || 'Unknown',
            msg.role || msg.agentRole || 'Agent',
            msg.round || 1,
            content,
          )
        }
      ...
      console.error('[Import] Ошибка импорта дебата:', err.message)
      skipped++
    }
  }

  res.json({ imported, skipped, total: debates.length })
})
...
app.post('/tournament/start', async (req, res) => {
  ...
  const { agents: agentIds, topic, rounds = 3 } = req.body

  if (!agentIds || agentIds.length < 2) {
    return res.status(400).json({ error: 'Нужно минимум 2 агента для турнира' })
  }
  ...
  const bracket = createTournamentBracket(agentIds, customAgents)
  bracket.topic = topic.trim()
  bracket.rounds_count = rounds
  activeTournaments.set(bracket.id, bracket)

  res.json({ tournamentId: bracket.id, bracket })
})

app.get('/tournament/:id', (req, res) => {
  const tournament = activeTournaments.get(req.params.id)
  if (!tournament) {
...
app.get('/tournament/:id/stream', async (req, res) => {
  const tournament = activeTournaments.get(req.params.id)
  if (!tournament) {
    return res.status(404).json({ error: 'Турнир не найден' })
  }
  ...
  try {
    for (let ri = 0; ri < tournament.rounds.length; ri++) {
      ...
      sendEvent(res, { type: 'tournament_round', round: ri + 1, totalRounds: tournament.rounds.length })
      ...
      for (let mi = 0; mi < round.length; mi++) {
        ...
        if (match.status === 'bye') {
          sendEvent(res, { type: 'match_end', matchId: match.id, winner: match.winner?.name, winnerId: match.winner?.id, verdict: 'BYE', round: ri + 1 })
          continue
        }

        if (!match.agent1 || !match.agent2) continue
        ...
        const result = await runTournamentMatch(
          ...
          tournament.topic,
          provider.name,
          tournament.model || null,
          customAgents,
          (event) => {
            if (!aborted) sendEvent(res, { ...event, matchId: match.id })
          },
          tournament.rounds_count || 3,
        )
        ...
        const nextRound = tournament.rounds[ri + 1]
        if (nextRound && match.winner) {
          const targetMatch = Math.floor(mi / 2)
          if (nextRound[targetMatch]) {
            if (mi % 2 === 0) {
              nextRound[targetMatch].agent1 = match.winner
            } else {
              nextRound[targetMatch].agent2 = match.winner
            }
          }
        }
      }
    ...
    tournament.status = aborted ? 'stopped' : 'completed'

    const finalMatch = tournament.rounds[tournament.rounds.length - 1]?.[0]
    tournament.champion = finalMatch?.winner?.name || null
    ...
    sendEvent(res, { type: 'error', message: formatApiError(err) })
  }
